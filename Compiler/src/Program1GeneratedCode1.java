        ...
        switch (s.kind()) {
            case BLOCK: {
                ...
                for (int i = 0; i < s.lengthOfBlock(); i++) {
                    ...
                    Statement sSub = s.removeFromBlock(i);
                    generateCodeForStatement(sSub, context, cp);
                    s.addToBlock(i, sSub);
                }
            ...
            case CALL: {
                ...
                String sLeaf = s.disassembleCall();

                if (sLeaf.equals("move")) {
                    cp.add(cp.length(), Instruction.MOVE.byteCode());
                } else if (sLeaf.equals("turnleft")) {
                    cp.add(cp.length(), Instruction.TURNLEFT.byteCode());
                } else if (sLeaf.equals("turnright")) {
                    cp.add(cp.length(), Instruction.TURNRIGHT.byteCode());
                } else if (sLeaf.equals("infect")) {
                    cp.add(cp.length(), Instruction.INFECT.byteCode());
                } else if (sLeaf.equals("skip")) {
                    cp.add(cp.length(), Instruction.SKIP.byteCode());
                } else if (context.hasKey(sLeaf)) {
                    generateCodeForStatement(context.value(sLeaf), context, cp);
                } else if (!context.hasKey(sLeaf)) {
                    Reporter.assertElseFatalError(true,
                            "Violation of: cannot use undefined instruction");
                } else {
                    Reporter.assertElseFatalError(true, "INVALID");
                }

                s.assembleCall(sLeaf);
                break;
            }
            default: {
